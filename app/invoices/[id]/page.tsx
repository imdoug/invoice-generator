...
export default async function InvoiceDetailPage({ params }: { params: { id: string } }) {
  ...
  if (!session || !session.user?.email) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center space-y-6 text-center">
        <p className="text-gray-600 text-lg">Not authenticated. Please log in to view invoices.</p>
        <Link href="/login">
  ...
  if (error || !invoice) {
    ...
    return notFound(); // Show 404 page if not found
  }
    ...
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow space-y-6">
        ...
        <div className="text-gray-700 space-y-2">
          <p><span className="font-semibold">Client Name:</span> {invoice.client_name}</p>
          <p><span className="font-semibold">Client Address:</span> {invoice.client_address}</p>
